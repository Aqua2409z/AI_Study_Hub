...
// 🛠️ Thêm icon MoreVertical từ lucide-react để làm menu ba chấm
            ...
            <div className="space-y-5">
              {[1, 2, 3].map((i) => (
                <div key={i} className={`flex gap-3 ${i % 2 === 0 ? "flex-row-reverse" : ""}`}>
                  {i % 2 !== 0 && (
                    <div className="size-8 shrink-0 rounded-xl bg-primary/10 text-primary/40 grid place-items-center mt-1 animate-pulse">
                      <Bot size={16} />
                    </div>
                  )}
                  <div className={`max-w-[70%] w-1/2 h-14 rounded-2xl bg-muted/40 border border-border/40 animate-pulse ${i % 2 === 0 ? "rounded-tr-sm" : "rounded-tl-sm"}`} />
                </div>
              ))}
            </div>
              ...
              {messages.map((m) => (
                ...
                >
                  ...
                  <div className={`max-w-[85%] ${m.senderRole === "USER" ? "items-end" : "items-start"} flex flex-col gap-1.5`}>
                    {m.senderRole === "USER" ? (
                      <div className="px-4 py-2.5 rounded-2xl rounded-tr-sm bg-primary text-primary-foreground text-sm whitespace-pre-wrap shadow-sm">
                        {m.content}
                      </div>
                    ) : (
                      <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-muted/40 border border-border/50 text-foreground text-sm whitespace-pre-wrap leading-relaxed">
                        {m.content}
                      </div>
                    )}
